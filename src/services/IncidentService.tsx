...
function dateToFirestoreTimestamp(date: Date): {
  seconds: number;
  nanos: number;
} {
...
export const saveUserToStorage = (
  user: Partial<UserStorageData> & { uid?: string; facilityName?: string }
): void => {
  try {
...
function mapApiIncident(api: APIIncident): Incident {
  ...
  const result: Incident = {
    ...
    assignedTo: api.assignedTo
      ? createUserFromApi(api.assignedTo, api.facility || api.location)
      : undefined,
    reportedBy: api.reportedBy
      ? createUserFromApi(api.reportedBy, api.facility || api.location)
      : createSystemUser(),

    // ✅ FIXED: Safe date parsing
    createdAt: parseDate(api.createdAt) || new Date(),
    updatedAt:
      parseDate(api.updatedAt) || parseDate(api.createdAt) || new Date(),
    ...
    escalationLevel:
      typeof api.escalationLevel === "number" ? api.escalationLevel : 0,
...
export const getIncidentById = async (id: string): Promise<Incident> => {
  ...
  return handleApiCall(async () => {
    ...
    try {
      const response = await axios.get<APIIncident>(url, {
      ...
      try {
        ...
        console.error(
          "❌ Raw API data that failed mapping:",
          JSON.stringify(response.data, null, 2)
        );
        ...
        if (mappingError instanceof Error) {
          throw new Error(
            `Failed to process incident data: ${mappingError.message}`
          );
        } else {
          throw new Error(
            "Failed to process incident data: Unknown mapping error"
          );
        }
      }
      ...
      if (
        ...
        const errorResponse = axiosError as {
        ...
        if (errorResponse.response?.status === 500) {
          ...
          throw new Error(
            `Server error while fetching incident. Please try again later.`
          );
        }
      }
      throw axiosError;
...
export const getFacilityIncidents = async (
  ...
  if (!facility || facility === "all") {
    throw new Error(
      "getFacilityIncidents requires a valid facility identifier"
    );
  }
...
export const assignIncident = async (
  ...
  return handleApiCall(async () => {
    ...
    await axios.put(`${API_BASE_URL}/incidents/${id}/assign`, payload, {
      headers: {
        ...getAuthHeader(),
        "Content-Type": "application/json",
      },
...
export const addNoteToIncident = async (
  ...
  return handleApiCall(async () => {
    debugLog("Adding note to incident", {
      incidentId,
