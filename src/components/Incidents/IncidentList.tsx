    ...
    <div className="p-6">
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-slate-400">Monitor and manage warehouse incidents across all facilities</p>
        </div>
        <Button 
      ...
      <div className="flex items-center justify-between mb-4">
        <div className="flex items-center space-x-4">
          <span className="text-sm text-slate-400">
            Showing {filteredIncidents.length} of {state.incidents.length} incidents
          </span>
        ...
        <div className="flex items-center space-x-2">
          ...
          <Badge variant="yellow" size="sm">
            {state.incidents.filter(i => ['acknowledged', 'in-progress'].includes(i.status)).length} Active
          </Badge>
          <Badge variant="red" size="sm">
            {state.incidents.filter(i => new Date() > i.slaDeadline && !['resolved', 'closed'].includes(i.status)).length} Overdue
          </Badge>
        </div>
      </div>
