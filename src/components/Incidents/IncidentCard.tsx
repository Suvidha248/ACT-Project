      ...
      >
            ...
            <div className="flex items-center space-x-3 mb-2">
              ...
              {incident.escalationLevel > 0 && (
                <div className="flex items-center space-x-1 text-red-400">
                  ...
                  <span className="text-xs font-medium">Escalated ({incident.escalationLevel})</span>
                </div>
              )}
              {isOverdue && (
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 text-sm text-slate-400 mb-4">
          ...
          <div className="flex items-center space-x-2">
            ...
            <span className="font-mono text-xs">{formatDistanceToNow(incident.createdAt, { addSuffix: true })}</span>
          </div>
          
          <div className="flex items-center space-x-2">
            <User className="w-4 h-4 text-teal-400" />
            <span className="font-mono text-xs">{incident.assignedTo?.name || 'Unassigned'}</span>
          </div>
          
          <div className="flex items-center space-x-2">
            <MessageSquare className="w-4 h-4 text-teal-400" />
            <span className="font-mono text-xs">{incident.notes.length} notes</span>
          </div>
        </div>
        ...
        <div className="flex items-center justify-between pt-4 border-t border-white/10">
          <div className="flex items-center space-x-4 text-xs text-slate-500">
            <span className="font-mono">Reporter: {incident.reportedBy.name}</span>
          </div>
          
          <div className={`text-xs font-mono ${isOverdue ? 'text-red-400 font-semibold' : 'text-slate-500'}`}>
