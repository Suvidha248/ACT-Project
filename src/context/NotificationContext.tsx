...
export interface NotificationContextType extends NotificationState {
  ...
  addNotification: (
    notification: Omit<Notification, "id" | "timestamp" | "recipients">
  ) => Promise<void>;
}
...
export const NotificationProvider: React.FC<{ children: React.ReactNode }> = ({
      ...
      if (data) {
        const notificationsList: Notification[] = Object.entries(data).map(
          ([key, value]) => ({
            ...
            actionUrl:
              value.actionUrl ||
              (value.incidentId ? `/incidents/${value.incidentId}` : undefined),
          })
        );
  ...
  useEffect(() => {
    ...
    const pubsubNotificationsRef = ref(
      rtdb,
      `pubsub-notifications/${profile.facilityName.toLowerCase()}`
    );

    const unsubscribePubSub = onValue(pubsubNotificationsRef, (snapshot) => {
      const data = snapshot.val() as Record<
        string,
        PubSubNotificationData
      > | null;
      if (data) {
        ...
        Object.values(data).forEach((pubsubNotif) => {
          if (
            pubsubNotif.payload &&
            pubsubNotif.timestamp > Date.now() - 30000
          ) {
            try {
  ...
  const addNotificationToPersonalFeed = async (
    notification: ParsedNotificationPayload
  ) => {
    if (!user?.uid) return;
    ...
    try {
      ...
      await push(userNotificationsRef, {
        ...
        actionUrl: notification.incidentId
  ...
  const markAsRead = async (id: string): Promise<void> => {
    try {
      ...
      const notificationRef = ref(
        rtdb,
        `notifications/${user.uid}/items/${id}`
      );
